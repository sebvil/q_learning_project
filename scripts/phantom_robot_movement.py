...
class PhantomRobotMovement(object):
    def __init__(self):
        ...
        rospy.Subscriber(
            ...
            RobotMoveDBToBlock,
            self.prepare_to_take_robot_action,
        )

        # ROS subscribe to the Gazebo topic publishing the locations of the models
        rospy.Subscriber(
            "/gazebo/model_states", ModelStates, self.model_states_received
        )
    ...
    def execute_robot_action(self):
        ...
        if len(self.robot_action_queue) > 0:
            ...
            pt = Point(
                x=(
                    self.current_numbered_block_locations[
                        robot_action_to_take.goal_block_num
                    ].x
                    + 0.6
                ),
                y=self.current_numbered_block_locations[
                    robot_action_to_take.goal_block_num
                ].y,
                z=self.current_numbered_block_locations[
                    robot_action_to_take.goal_block_num
                ].z,
            )
            ...
            p = Pose(position=pt, orientation=self.quat_orientation_of_dbs)
            t = Twist(linear=Vector3(0, 0, 0), angular=Vector3(0, 0, 0))
            m_name = self.db_model_names[robot_action_to_take.robot_db]

            robot_db_model_state = ModelState(
                model_name=m_name, pose=p, twist=t
            )
    ...
    def model_states_received(self, data):
        ...
        if len(self.robot_action_queue) > 0:
            ...
            for block_id in self.numbered_block_model_names:
                block_idx = data.name.index(
                    self.numbered_block_model_names[block_id]
                )
                self.current_numbered_block_locations[block_id] = data.pose[
                    block_idx
                ].position
            ...
            for robot_db in self.db_model_names:
                ...
                self.current_db_locations[robot_db] = data.pose[
                    db_idx
                ].position
